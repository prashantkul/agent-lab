...
def send_submission_notification(
    ...
    subject = f"[Course Review] New {submission_type.replace('_', ' ')} submission: {module_name}"

    clarity_stars = "N/A" if clarity_rating is None else "&#9733;" * clarity_rating + f" ({clarity_rating}/5)"
    difficulty_stars = "N/A" if difficulty_rating is None else "&#9733;" * difficulty_rating + f" ({difficulty_rating}/5)"
    time_str = f"{time_spent} minutes" if time_spent else "Not specified"

    html = f"""
    <div style="font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif; max-width: 600px; margin: 0 auto;">
        <div style="background: linear-gradient(135deg, #1e3a8a 0%, #3b82f6 100%); padding: 30px; border-radius: 12px 12px 0 0;">
            <h2 style="color: white; margin: 0;">New Review Submission</h2>
        </div>
        <div style="background: white; padding: 30px; border: 1px solid #e2e8f0; border-top: none; border-radius: 0 0 12px 12px;">
            <table style="border-collapse: collapse; width: 100%;">
                ...
                <tr><td style="padding: 8px; border-bottom: 1px solid #eee;"><strong>Type</strong></td><td style="padding: 8px; border-bottom: 1px solid #eee;">{submission_type.replace('_', ' ').title()}</td></tr>
                <tr><td style="padding: 8px; border-bottom: 1px solid #eee;"><strong>GitHub</strong></td><td style="padding: 8px; border-bottom: 1px solid #eee;"><a href="{github_link}">{github_link}</a></td></tr>
                <tr><td style="padding: 8px; border-bottom: 1px solid #eee;"><strong>Clarity Rating</strong></td><td style="padding: 8px; border-bottom: 1px solid #eee;">{clarity_stars}</td></tr>
                <tr><td style="padding: 8px; border-bottom: 1px solid #eee;"><strong>Difficulty Rating</strong></td><td style="padding: 8px; border-bottom: 1px solid #eee;">{difficulty_stars}</td></tr>
            ...
            <h3 style="color: #1e293b; margin-top: 20px;">Comments</h3>
